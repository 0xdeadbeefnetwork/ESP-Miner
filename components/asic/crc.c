...
{
    if (data == NULL) {
        ESP_LOGE("CRC", "Data pointer is NULL");
        return 0;
    }
...
{
    if (buffer == NULL) {
        ESP_LOGE("CRC", "Buffer pointer is NULL");
        return 0;
    }
...
uint16_t crc16_false(uint8_t *buffer, uint16_t len)
{
    ...
    uint16_t crc = 0xffff; // Initialize CRC to 0xFFFF for CRC-16/CCITT-FALSE
