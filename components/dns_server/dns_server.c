...
static char *parse_dns_name(char *raw_name, char *parsed_name, size_t parsed_name_max_len)
{
    ...
    do {
        ...
        if (name_len > parsed_name_max_len || sub_name_len >= parsed_name_max_len - (name_itr - parsed_name)) {
            ESP_LOGE(TAG, "Buffer overflow detected while parsing DNS name");
            return NULL;
        }
...
static int parse_dns_request(char *req, size_t req_len, char *dns_reply, size_t dns_reply_max_len, dns_server_handle_t h)
{
    if (req_len > dns_reply_max_len) {
        ESP_LOGE(TAG, "Request length exceeds buffer size");
        return -1;
    }
    ...
    if (reply_len > dns_reply_max_len) {
        ESP_LOGE(TAG, "DNS reply length exceeds buffer size");
        return -1;
    }
    ...
    while (handle->started) {
        ...
        while (handle->started) {
            ...
            if (len < 0) {
                ...
                if (sock < 0) {
                    ESP_LOGE(TAG, "Failed to re-create socket: errno %d", errno);
                    break;
                }
                continue;
            }
